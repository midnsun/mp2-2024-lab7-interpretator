...
class interpretator {
	...
	void process(std::vector<std::string>& source) { // предобработка кода дл€ исполнени€
		...
		// раздел€ютс€ int, double, void, if, else, while, return. scan, print - обычные операторы
		// команда завершаетс€ или символом { или символом ;. ƒругого не дано
		...
		for (size_t i = 0; i < sourceCode.length(); ++i) {
			if (sourceCode[i] == '{' || sourceCode[i] == ';') {
				lexComand.clear();
				// command processing - разделение команды на составл€ющие лексемы
				command = "";
			}
			else 
